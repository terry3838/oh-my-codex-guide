...
sequenceDiagram
    ...
    participant NotifyHook as notify-hook.js
    participant TmuxHook as tmux-hook 엔진
    participant PluginDispatch as 플러그인 디스패치
    participant Plugin as .omx/hooks/*.mjs
    participant State as .omx/state/

    Codex->>NotifyHook: turn-complete 이벤트<br/>(JSON payload)
    NotifyHook->>NotifyHook: 모드 상태 확인<br/>(ralph/ultrawork/team 활성?)
    alt tmux-hook 활성화 (allowed_modes 매칭)
        NotifyHook->>TmuxHook: 훅 엔진 실행
        TmuxHook->>TmuxHook: cooldown 확인<br/>max_injections 확인<br/>스크롤 상태 확인
        TmuxHook->>Codex: tmux send-keys<br/>"Continue from current mode state."
    end
    alt OMX_HOOK_PLUGINS=1
        NotifyHook->>PluginDispatch: 플러그인 이벤트 디스패치
        PluginDispatch->>Plugin: onHookEvent(event, sdk) 호출
        Plugin-->>PluginDispatch: 처리 완료
        PluginDispatch->>State: 플러그인 상태 기록
    end
    NotifyHook->>State: .omx/logs/ JSONL 기록
```
...
graph LR
    subgraph "Codex 에이전트"
        CODEX[Codex CLI<br/>에이전트]
    end

    subgraph "MCP 서버층"
        STATE[omx_state<br/>state-server.js]
        MEM[omx_memory<br/>memory-server.js]
        INTEL[omx_code_intel<br/>code-intel-server.js]
        TRACE[omx_trace<br/>trace-server.js]
        TEAM[omx_team_run<br/>team-server.js]
    end

    subgraph ".omx/ 파일시스템"
        STATE_FILES[".omx/state/\n*-state.json"]
        MEM_FILES[".omx/project-memory.json\n.omx/notepad.md"]
        LOG_FILES[".omx/logs/\nturns-*.jsonl"]
    end
    ...
    CODEX <-->|MCP protocol| STATE
    CODEX <-->|MCP protocol| MEM
    CODEX <-->|MCP protocol| INTEL
    CODEX <-->|MCP protocol| TRACE
    CODEX <-->|MCP protocol| TEAM
...
```mermaid
flowchart TD
    A[omx setup 실행] --> B{scope?}
    B -->|project scope| C[templates/AGENTS.md 읽기]
    B -->|user scope| D[프로젝트 AGENTS.md 변경 없음]
    C --> E[경로 재작성<br/>~/.codex → ./.codex<br/>~/.agents → ./.agents]
    E --> F[<cwd>/AGENTS.md 생성/갱신]

    G[omx 실행] --> H[세션 ID 생성]
    H --> I[generateOverlay 호출]
    I --> J[병렬 데이터 수집]
    J --> J1[활성 모드 상태]
    J --> J2[notepad PRIORITY]
    J --> J3[project-memory 요약]
    J --> J4[코드베이스 맵]
    J --> J5[ralph 게이트 확인]
    J1 & J2 & J3 & J4 & J5 --> K[오버레이 콘텐츠 조립<br/>최대 3500자]
    K --> L[writeSessionModelInstructionsFile]
    F --> L
    L --> M[.omx/state/sessions/sessionId/AGENTS.md 생성]
    M --> N[Codex 실행<br/>-c model_instructions_file=세션경로/AGENTS.md]
    N --> O[에이전트 세션 활성]
    O --> P[세션 종료]
    P --> Q[removeSessionModelInstructionsFile<br/>세션 파일 정리]
```
...
```mermaid
flowchart TD
    A[omx setup 실행] --> B[기존 config.toml 읽기<br/>없으면 빈 문자열]
    B --> C{기존 OMX 마커 블록<br/>존재?}
    C -->|yes| D[stripExistingOmxBlocks<br/>마커 블록 제거]
    C -->|no| E
    D --> E[stripOmxTopLevelKeys<br/>기존 OMX 루트키 제거]
    E --> F[stripOrphanedOmxSections<br/>고아 mcp_servers.omx_* 제거]
    F --> G[upsertFeatureFlags<br/>[features] 섹션 upsert]
    G --> H{root model이<br/>legacy 버전?}
    H -->|gpt-5.3-codex| I[대화형: 업그레이드 여부 확인]
    H -->|other| J
    I --> J[getOmxTopLevelLines<br/>새 루트키 생성]
    J --> K[getOmxTablesBlock<br/>MCP + agents + tui 블록 생성]
    K --> L[최종 조립:<br/>topLines + existing + tablesBlock]
    L --> M{내용 변경됨?}
    M -->|yes| N[기존 파일 백업<br/>.omx/backups/setup/timestamp/]
    N --> O[config.toml 쓰기]
    M -->|no| P[변경 없음 - 건너뜀]
```
...
sequenceDiagram
    ...
    participant AGENTS as AGENTS.md<br/>키워드 감지
    participant Skill as ~/.agents/skills/<br/>ralph/SKILL.md

    User->>Codex: "ralph: 이 기능 완성될 때까지 계속해"
    Codex->>AGENTS: 키워드 감지 규칙 확인
    AGENTS->>AGENTS: "ralph" 키워드 → $ralph 스킬 매핑
    Codex->>Skill: ./.agents/skills/ralph/SKILL.md 읽기
    Skill-->>Codex: 스킬 지시 내용 로드
    Codex->>Codex: 스킬 워크플로우 실행<br/>(ralph 루프 시작)

    Note over Codex: 또는 명시적 호출:
    User->>Codex: "$ralph 계속 진행해"
    Codex->>Skill: 스킬 직접 로드
```
...
```mermaid
flowchart TD
    A["omx team N:role '작업'"] --> B{OMX_TEAM_WORKER_CLI_MAP<br/>설정됨?}
    B -->|yes| C[MAP에서 워커 인덱스별 CLI 읽기]
    B -->|no| D{OMX_TEAM_WORKER_CLI?}
    D -->|codex| E[codex CLI 강제]
    D -->|claude| F[claude CLI 강제]
    D -->|auto 또는 없음| G{워커 --model에<br/>'claude' 포함?}
    G -->|yes| F
    G -->|no| E
    ...
    E --> I[codex 워커 프로세스 생성<br/>OMX_TEAM_WORKER_LAUNCH_ARGS 적용<br/>model_reasoning_effort 롤별 설정]
    F --> J[claude 워커 프로세스 생성<br/>기본 settings.json 사용<br/>--model/--config/--effort 무시]

    I --> K[tmux 패인에 워커 실행<br/>또는 LAUNCH_MODE=prompt]
    J --> K
```
...
```mermaid
flowchart TD
    A[작업 요청] --> B[에이전트 롤 결정<br/>예: planner, executor, writer]
    B --> C[AGENT_DEFINITIONS 조회<br/>posture / modelClass / reasoningEffort]

    C --> D{posture?}
    D -->|frontier-orchestrator| E["<posture_overlay>\n라우팅 우선, 위임 적극적\n가정에 도전, 경계 보존\n</posture_overlay>"]
    D -->|deep-worker| F["<posture_overlay>\n직접 실행 우선\n탐색 후 최소 변경\n검증 필수 (테스트/빌드)\n</posture_overlay>"]
    D -->|fast-lane| G["<posture_overlay>\n빠른 분류/검색\n확장 시 에스컬레이션\n보수적 응답\n</posture_overlay>"]

    E --> H[네이티브 에이전트 .toml 생성<br/>model_reasoning_effort 포함]
    F --> H
    G --> H

    H --> I["config.toml [agents.<name>]에 등록\nconfig_file = ~/.omx/agents/<name>.toml"]
    I --> J[Codex가 /prompts:<name> 호출 시<br/>해당 .toml 로드]
```
