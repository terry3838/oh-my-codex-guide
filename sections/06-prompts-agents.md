...
```mermaid
flowchart TD
    Task["작업 요청"] --> RoleID["역할 식별\n(role)"]
    RoleID --> PostureAssign["포스처 배정\n(posture)"]

    PostureAssign --> FO["frontier-orchestrator\n계획·라우팅 중심"]
    PostureAssign --> DW["deep-worker\n구현·실행 중심"]
    PostureAssign --> FL["fast-lane\n경량·탐색 중심"]

    FO --> FO_agents["planner\narchitect\ncritic\nanalyst"]
    DW --> DW_agents["executor\nbuild-fixer\ntest-engineer\nteam-executor"]
    FL --> FL_agents["explore\nwriter\nresearcher"]
```
