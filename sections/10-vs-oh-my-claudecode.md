...
flowchart LR
    subgraph OMC["oh-my-claudecode (OMC)"]
        ...
        OMC_Runtime["OMC 런타임"]
        OMC_CLI["Claude Code"]
        OMC_Agents["CLAUDE.md\n에이전트 정의"]
        OMC_Prompts["~/.claude/prompts/\n(에이전트 프롬프트)"]
        OMC_Skills["~/.claude/plugins/\n(스킬)"]
        OMC_State[".omc/\n(상태·메모리·로그)"]

        OMC_User --> OMC_Runtime
        OMC_Runtime --> OMC_CLI
        OMC_CLI --> OMC_Agents
        OMC_CLI --> OMC_Prompts
        OMC_CLI --> OMC_Skills
        OMC_Runtime --> OMC_State
    end
    ...
    subgraph OMX["oh-my-codex (OMX)"]
        ...
        OMX_Runtime["OMX 런타임"]
        OMX_CLI["Codex CLI"]
        OMX_Agents["AGENTS.md\n오케스트레이션 가이드"]
        OMX_Prompts["~/.codex/prompts/\n(에이전트 프롬프트)"]
        OMX_Skills["~/.agents/skills/\n(스킬)"]
        OMX_State[".omx/\n(상태·메모리·로그)"]

        OMX_User --> OMX_Runtime
        OMX_Runtime --> OMX_CLI
        OMX_CLI --> OMX_Agents
        OMX_CLI --> OMX_Prompts
        OMX_CLI --> OMX_Skills
        OMX_Runtime --> OMX_State
    end
```
...
```mermaid
flowchart LR
    OMC_Role["OMC: 역할(role)만"] --> OMC_Exec["executor → 실행"]

    OMX_Role["OMX: 역할(role)"] --> OMX_Posture["포스처(posture) 배정"]
    OMX_Posture --> FO["frontier-orchestrator\n(planner, architect)"]
    OMX_Posture --> DW["deep-worker\n(executor, build-fixer)"]
    OMX_Posture --> FL["fast-lane\n(explore, writer)"]
```
